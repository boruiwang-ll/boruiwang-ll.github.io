...
Singularity is an execution-mechanisms paper. MAST is a scheduling-policy paper. They occupy **different layers of the same stack** — and this isn't an accident. It's a direct consequence of building for a public cloud versus a private cloud. MAST itself confirms this: it cites Singularity and dismisses it in one sentence — *"the article does not disclose details about the scheduling part, but focuses more on how to provide elasticity to ML training jobs, and thus it's impossible for us to provide a concrete comparison."*

The two papers aren't competitors. They're complementary halves of the same unsolved problem.
...
The numbers back this up. MAST schedules tens of thousands of workloads daily across O(100,000) GPUs. GMS takes ~34 seconds to rank 6,000–10,000 workloads, with 8.8× headroom before hitting limits. RMS places a workload in 1.3–5.5ms. GPU allocation rate is **98%** fleet-wide. Before MAST, the most overloaded region had a GPU demand-to-supply ratio of 2.63 for high-priority workloads. After: **0.98**.
...
Who's closest to filling both layers? Arguably Google — GCP controls the TPU hardware, the JAX/XLA framework, *and* the data infrastructure, giving it visibility into both execution and decision layers simultaneously. Whether they've actually built the unified system these papers imply is needed is another question. But the structural prerequisites are there in a way they aren't for Azure (which can't touch customer data) or Meta (which doesn't need infrastructure-level transparency).
